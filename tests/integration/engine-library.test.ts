...
describe('WorkRail library engine', () => {
  ...
  it('lists workflows', async () => {
    await withEngine(async (engine) => {
      ...
      const bugInvestigation = listResult.value.workflows.find(w => w.workflowId === 'bug-investigation-agentic');
  ...
  it('starts a workflow and receives discriminated ok response', async () => {
    await withEngine(async (engine) => {
      const startResult = await engine.startWorkflow('bug-investigation-agentic');
