...
### Branch hygiene: cross-work contamination must be structurally impossible (Apr 18, 2026)

**The problem:** when multiple agents or a human+agent are working simultaneously, commits from one task can land on a branch that belongs to another task. This happened today: backlog/docs commits landed on `feat/daemon-notifications` and `fix/spawn-agent-result-handling` branches, contaminating those PRs with unrelated changes and causing a branch entanglement that required manual cleanup.

**Why it happened:** the main working tree's current branch changes as agents do their work, and any `git commit` in the main working tree goes to whatever branch happens to be checked out. The backlog/docs commits were made against the current branch without checking what it was.

**The structural fix (three rules, all must be enforced):**

**Rule 1: The main working tree is read-only for commits.**
The main working tree (`~/git/personal/workrail/`) is only for reading state (git log, status, diff). All commits go through worktrees created specifically for the task. If something needs to be committed and there's no worktree for it, create a clean branch first.

**Rule 2: Docs/backlog updates get their own dedicated branch.**
Backlog additions, design decisions, and session notes are committed to a `docs/session-YYYY-MM-DD` branch that is created fresh from main at the start of each session and PRed at the end. Never appended to a code branch.

**Rule 3: Worktree scope is declared and enforced.**
When WorkTrain creates a worktree for a task, it records the intended scope (which files/directories are in scope). Any commit touching files outside that scope is flagged before it lands. This is a WorkTrain-level guard, not git-level -- git doesn't know about task scope, but WorkTrain does.

**Implementation path:**
- Rule 1 and 2 are process rules enforceable today via daemon-soul.md additions
- Rule 3 requires a `WorktreeManifest` concept in the coordinator/daemon: when a task is spawned with `--isolation worktree`, record the expected file scope in `~/.workrail/worktrees/<id>/manifest.json`. The pre-commit check reads this and warns (or blocks) out-of-scope commits.
- Eventually: WorkTrain's `worktrain worktree` commands enforce this automatically

**Add to daemon-soul.md immediately:**
```
## Branch hygiene (non-negotiable)
- NEVER commit to the main working tree. All commits go through a task-specific worktree.
- NEVER commit backlog.md or docs/ changes to a code branch. Create docs/session-YYYY-MM-DD from main first.
- Before any git commit, run `git branch --show-current` and verify the branch matches the task.
```
