...
- **Rule**: Declare requireConfirmation on the work that needs review. Autonomous execution evaluates its condition after validating that work against the merged submission context.
- **Why**: A valid direct or corrected submission parks at the declared gate before advancing. WorkRail retains the accepted notes, artifacts and context; only the trusted host resolver can authorize the successor.
- **Enforced by**: advisory
