...
export class NodeProcessSignals implements ProcessSignals {
  on(signal: ProcessSignal, handler: () => void | Promise<void>): void {
    ...
    // shared DI singleton -- wireShutdownHooks() calls on() for SIGINT/SIGTERM/SIGHUP.
    // With process.once, the second caller's registrations would be silently dropped
    // after the first signal fires. Double-invocation is prevented by the shutdownStarted
    // latch in each caller, not by once-semantics at the OS listener level.
