...
export function createInspector(engine: AnswerEngine, enrollment: HostEnrollment): HostInspectorPort {
  return {
    ...
    async inspect(read,signal){
      ...
      if(loaded.kind!=='loaded'||read!==capability(engine,loaded.state,'read'))return {kind:'unavailable',reason:'invalid_scope'};
    ...
    async inspectReceipt(read,receipt,signal,cursor){
      if(signal.aborted)return {kind:'refused',reason:'storage_unavailable'};
      const loaded=await readHostState(engine,enrollment);
      if(loaded.kind!=='loaded')return {kind:'refused',reason:'storage_unavailable'};
      const state=loaded.state;
      if(read!==capability(engine,state,'read'))return {kind:'refused',reason:'invalid_scope'};
      const record=state.records.find(r=>(r.kind==='committed'||r.kind==='rejected')&&r.receipt===receipt);
      if(!record||(record.kind!=='committed'&&record.kind!=='rejected'))return {kind:'refused',reason:'invalid_scope'};
      const text=record.kind==='committed'?JSON.stringify({notes:record.notes}):record.rawAnswer;
      let offset=0;
      if(cursor){
        const parts=cursor.split(':');const value=Number(parts[0]);
        if(parts.length!==2||!Number.isSafeInteger(value)||value<0||parts[1]!==capability(engine,state,'cursor',`${receipt}:${value}`))return {kind:'refused',reason:'invalid_scope'};
        offset=value;
      }
      const bytes=Buffer.from(text);if(offset>bytes.length)return {kind:'refused',reason:'invalid_scope'};
