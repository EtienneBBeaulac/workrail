...
export async function createAnswerHost(config: AnswerHostConfig, lifetime: AbortSignal): Promise<CreateAnswerHostResult> {
  if(lifetime.aborted)return {kind:'refused',reason:'storage_unavailable',detail:'Host creation cancelled'};
  if(![config.storage.journalRootDir,config.storage.hostIndexRootDir,config.keyringPath,config.workflowStoragePath].every(isAbsolute))return {kind:'refused',reason:'missing_authority',detail:'Explicit absolute authority paths required'};
  const composition=await composeAnswerEngine(config);
  if(composition.kind!=='ready')return {kind:'refused',reason:'storage_unavailable',detail:composition.detail};
  const engine=composition;
  const shutdown=new AbortController();
  let closed=false;
  const active=new Set<Promise<unknown>>();
  const available=(s:AbortSignal)=>!closed&&!lifetime.aborted&&!s.aborted;
  const track=async<T>(operation:Promise<T>):Promise<T>=>{active.add(operation);try{return await operation;}finally{active.delete(operation);}};
  ...
  const hydrator:TrustedAnswerScheduler['hydrator']={
    ...
    async hydrate(raw,signal){
      if(!available(signal))return {kind:'refused',reason:'storage_unavailable',detail:'Host unavailable'};
      ...
      return loaded.kind==='loaded'?{kind:'hydrated',enrollment}:{kind:'refused',reason:loaded.reason,detail:loaded.detail};
    },
  };
  function runner(enrollment:HostEnrollment,owner:OwnerFence):BoundTurnRunner {
    ...
    async function turn(signal:AbortSignal):Promise<TurnOutcome>{
      ...
      if(recovered.kind==='refused'&&recovered.reason==='stopped'){
        ...
        if(stop?.kind==='stopped')return {kind:'stopped',execution:enrollment.execution,reason:stop.reason,detail:stop.detail};
      }
      if(recovered.kind==='refused')return recovered.reason==='stale_owner'?{kind:'stale_owner'}:{kind:'refused',reason:'storage_unavailable',detail:recovered.reason};
      if(recovered.kind==='settled'){
        const result=recovered.result;
        if(result.kind==='replay'||result.kind==='recorded')return {kind:'settled',receipt:result.receipt,view:recovered.view};
        return {kind:'refused',reason:'dispatch_refused',detail:result.kind};
      }
      if(recovered.kind==='reconciling')return {kind:'unconfirmed',uncertainty:{stage:'commit_or_dispatch',invocation:recovered.invocation}};
      let prepared:PreparedAnswer|undefined;
      let response:CapturedResponse|undefined;
      if(recovered.kind==='replay')prepared=recovered.answer;
      else if(recovered.kind==='prepare_response')response=recovered.response;
      else {
        ...
        if(delivery.kind==='stale_owner')return delivery;
        if(delivery.kind==='unconfirmed')return {kind:'unconfirmed',uncertainty:{stage:'delivery',execution:enrollment.execution,reply:recovered.view.reply}};
        if(delivery.kind==='refused')return {kind:'refused',reason:'delivery_refused',detail:delivery.reason};
        if(!available(signal))return {kind:'cancelled'};
        let completion:Awaited<ReturnType<typeof config.model.generate>>;
        try{completion=await config.model.generate({instruction:recovered.view.instruction,issues:recovered.view.issues,retainedSummaries:recovered.view.retained},signal);}
        catch(error){return signal.aborted?{kind:'cancelled'}:{kind:'refused',reason:'model_unavailable',detail:String(error)};}
        if(completion.kind==='cancelled')return {kind:'cancelled'};
        if(completion.kind==='unavailable')return {kind:'refused',reason:'model_unavailable',detail:completion.detail};
        const captured=await p.journal.captureResponse(delivery.delivery,completion.response,owner,signal);
        if(captured.kind==='stale_owner')return captured;
        if(captured.kind==='refused')return {kind:'refused',reason:'capture_refused',detail:captured.reason};
        if(captured.kind==='unconfirmed')return {kind:'unconfirmed',uncertainty:{stage:'capture',execution:enrollment.execution,delivery:delivery.delivery}};
        response=captured.response;
      }
      if(!prepared&&response){
        const result=await p.journal.prepare(response,owner,signal);
        if(result.kind==='rejected')return {kind:'rejected',receipt:result.receipt,correctionView:result.view};
        if(result.kind==='refused')return result.reason==='stale_owner'?{kind:'stale_owner'}:{kind:'refused',reason:'prepare_refused',detail:result.reason};
        if(result.kind==='unconfirmed')return {kind:'unconfirmed',uncertainty:{stage:'prepare',execution:enrollment.execution,delivery:response.delivery,response:response.response}};
        prepared=result.answer;
      }
      if(!prepared)return {kind:'refused',reason:'prepare_refused',detail:'No prepared response'};
      ...
      switch(result.kind){
        ...
        case 'replay':{
          const latest=await readHostState(engine,enrollment);const view=latest.kind==='loaded'?await workView(engine,latest.state):undefined;
    ...
    return {execution:enrollment.execution,async runTurn(signal){
      ...
      try{return await track(turn(combined));}catch(error){return {kind:'refused',reason:'storage_unavailable',detail:String(error)};}finally{running=false;}
    }};
  }
  type RecoveryMode={kind:'trusted'}|{kind:'unowned'}|{kind:'conditional';expected:OwnerFence};
  async function recover(raw:unknown,mode:RecoveryMode,signal:AbortSignal):Promise<RecoverHostSessionResult|ClaimUnownedResult>{
    ...
    if(hydrated.kind!=='hydrated')return hydrated;
    ...
    return j.locked<RecoverHostSessionResult|ClaimUnownedResult>(signal,{kind:'refused',reason:'storage_unavailable',detail:'Cannot lock recovery'},async(state,lock)=>{
      ...
      if(view.kind==='unavailable')return {kind:'refused',reason:'storage_unavailable',detail:view.detail};
      if(view.kind==='finished'){
        if(view.execution.kind==='incomplete')return {kind:'stopped',execution:enrollment.execution,reason:view.execution.reason,detail:view.execution.detail,read:view.read};
      ...
      if(mode.kind==='conditional'&&!owns(state,mode.expected))return {kind:'refused',reason:'ownership_changed',detail:'Expected owner is no longer current'};
      if(mode.kind==='unowned'&&state.owned)return {kind:'busy',detail:'Execution has an owner'};
      const owner={execution:enrollment.execution,epoch:state.epoch+1n} as OwnerFence;
      if(!await j.append(state,lock,{kind:'owner_acquired',epoch:owner.epoch.toString()},signal))return {kind:'unconfirmed',reason:'commit_uncertain'};
  ...
  const scheduler:TrustedAnswerScheduler={
    ...
    async enroll(input,signal){
      ...
      if(!parsed.success)return {kind:'refused',reason:'unsupported_workflow',detail:'Invalid host work request'};
      let raw:unknown;
      try{raw=JSON.parse(await readFile(join(config.workflowStoragePath,parsed.data.workflowId+'.json'),'utf8'));}catch{return {kind:'refused',reason:'unsupported_workflow',detail:'Cannot load requested workflow'};}
      if(!hasWorkflowDefinitionShape(raw)||raw.id!==input.workflowId||!raw.steps.every(s=>isStandardStepDefinition(s)&&!s.requireConfirmation&&!s.outputContract&&!s.validationCriteria&&!s.assessmentRefs&&!s.runCondition))return {kind:'refused',reason:'unsupported_workflow',detail:'This build enrolls linear notes workflows without gates or output contracts'};
      ...
      const started=await executeStartWorkflow({...engine,fallbackWorkflowReader:{getWorkflowById:async id=>id===workflow.definition.id?workflow:null},sessionStore:{load:id=>engine.sessionStore.load(id),loadValidatedPrefix:id=>engine.sessionStore.loadValidatedPrefix(id),append:(lock,plan)=>{
        ...
        if(run?.kind!=='run_started'||node?.kind!=='node_created'||!available(signal))return errAsync({code:'SESSION_STORE_IO_ERROR' as const,message:'Enrollment cancelled or incomplete'});
        enrollment={execution:lock.sessionId as string as ExecutionRef,recovery} as HostEnrollment;
        const state={enrollment,run};
        return engine.sessionStore.append(lock,{...plan,events:[...plan.events,hostEvent(engine,state,{kind:'enrolled',recovery,initialNode:node.scope.nodeId},plan.events.length),hostEvent(engine,state,{kind:'owner_acquired',epoch:'1'},plan.events.length+1)]});
      }}},parsed.data,{triggerSource:'daemon'});
      if(started.isErr()||!enrollment)return enrollment?{kind:'unconfirmed',reason:'commit_uncertain',pointer:hydrator.dehydrate(enrollment)}:{kind:'refused',reason:started.isErr()&&['pinned_workflow_store_failed','snapshot_creation_failed','session_append_failed','keyring_load_failed'].includes(started.error.kind)?'storage_unavailable':'initialization_failed',detail:started.isErr()?started.error.kind:'Missing enrollment'};
      const loaded=await readHostState(engine,enrollment);
      const view=loaded.kind==='loaded'?await workView(engine,loaded.state):undefined;
      if(!view||view.kind==='unavailable')return {kind:'unconfirmed',reason:'commit_uncertain',pointer:hydrator.dehydrate(enrollment)};
    ...
    async close(signal){closed=true;shutdown.abort();if(signal.aborted)return {kind:'incomplete',reason:'cancelled',detail:'Close cancelled'};return active.size?{kind:'incomplete',reason:'work_in_flight',detail:'Operations are settling'}:{kind:'closed'};},
  };
