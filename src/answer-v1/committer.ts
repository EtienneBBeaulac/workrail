...
export class AnswerCommitter implements FencedAnswerCommitter {
    ...
    async commit(answer: PreparedAnswer, owner: OwnerFence, signal: AbortSignal): Promise<FencedCommitResult> {
        ...
        if (!await j.fault('before_engine_transaction', signal))
            return { kind: 'commit_uncertain', invocation: answer.invocation };
        const result = await j.locked<FencedCommitResult>(signal, { kind: 'not_retained', reason: 'unavailable_storage' }, async (state, lock) => {
            ...
            if (snapshot.isErr() || !snapshot.value || pinned.isErr() || pinned.value?.sourceKind !== 'v1_pinned' || !hasWorkflowDefinitionShape(pinned.value.definition))
                return { kind: 'not_retained', reason: 'unavailable_storage' };
            ...
            const advanced = await executeAdvanceCore({ answerOwner:owner, mode: { kind: 'fresh', sourceNodeId: asNodeId(state.node), snapshot: snapshot.value },
                ...
                lockedIndex: buildSessionIndex(sorted.value), ports: { ...j.engine, sessionStore: { append: (_lock, plan) => {
                            ...
                            if (next?.kind !== 'advance_recorded' || next.data.outcome.kind !== 'advanced')
                                return errAsync({ code: 'SESSION_STORE_INVARIANT_VIOLATION' as const, message: 'Unsupported engine outcome' });
