...
export type HostEvent = Extract<DomainEventV1, { kind: 'answer_host_recorded' }>;
export type HostState = Readonly<{
  truth: LoadedSessionTruthV2;
  enrollment: HostEnrollment;
  run: Extract<DomainEventV1, { kind: 'run_started' }>;
  records: readonly AnswerHostRecord[];
  node: string;
  epoch: bigint;
  owned: boolean;
}>;
export type StateResult = { readonly kind: 'loaded'; readonly state: HostState } | { readonly kind: 'unavailable'; readonly reason: 'missing'|'corrupt'|'unsupported_version'|'storage_unavailable'; readonly detail: string };

export async function readHostState(engine: AnswerEngine, enrollment: HostEnrollment): Promise<StateResult> {
  if (!/^sess_[a-z0-9]+$/.test(enrollment.execution)) return { kind: 'unavailable', reason:'corrupt', detail: 'Invalid execution locator' };
  const loaded = await engine.sessionStore.load(asSessionId(enrollment.execution));
  if (loaded.isErr()) return { kind: 'unavailable',
  ...
  if (entries.length !== 1 || entry?.data.kind !== 'enrolled' || entry.data.recovery !== enrollment.recovery) return { kind: 'unavailable', reason:'missing', detail: 'Unknown enrollment' };
  const run = loaded.value.events.find((e): e is HostState['run'] => e.kind === 'run_started' && e.scope.runId === entry.scope.runId);
  if (!run) return { kind: 'unavailable', reason:'corrupt', detail: 'Missing engine run' };
  ...
  return { kind: 'loaded', state: { truth: loaded.value, enrollment, run, records,
...
export function capability(engine: AnswerEngine, state: Pick<HostState,'enrollment'|'run'>, role: string, ref = ''): string {
  ...
  return `ah1.${Buffer.from(signature).toString('base64url')}`;
}
...
export async function workView(engine: AnswerEngine, state: HostState, node = state.node): Promise<WorkView | {kind:'unavailable';detail:string}> {
  ...
  if (stopped?.kind === 'stopped') return { kind:'finished',read,retained,execution:{kind:'incomplete',reason:stopped.reason,detail:stopped.detail},taskOutcome:'unknown' };
  const event = state.truth.events.find(e => e.kind === 'node_created' && e.scope.nodeId === node && e.scope.runId === state.run.scope.runId);
  if (event?.kind !== 'node_created') return { kind:'unavailable',detail:'Missing current engine node' };
  const snapshot = await engine.snapshotStore.getExecutionSnapshotV1(event.data.snapshotRef);
  if (snapshot.isErr() || !snapshot.value) return { kind:'unavailable',detail:'Missing engine snapshot' };
  const pending = derivePendingStep(snapshot.value.enginePayload.engineState);
  if (!pending) {
    if (!state.truth.events.some(e => e.kind === 'run_completed' && e.scope.runId === state.run.scope.runId)) return {kind:'unavailable',detail:'Engine neither pending nor completed'};
  ...
  if (pinned.isErr() || pinned.value?.sourceKind !== 'v1_pinned' || !hasWorkflowDefinitionShape(pinned.value.definition)) return {kind:'unavailable',detail:'Missing pinned workflow'};
  ...
  if (rendered.isErr()) return {kind:'unavailable',detail:rendered.error.message};
