...
export type PreparedRecord = Extract<AnswerHostRecord,{kind:'prepared'}>;
...
export class SessionJournal implements InvocationJournal {
  constructor(readonly engine: AnswerEngine, readonly enrollment: HostEnrollment, readonly config: AnswerHostConfig,
    readonly available: (signal: AbortSignal) => boolean) {}
  ...
  async locked<T>(signal: AbortSignal, failure: T, fn: (state: HostState, lock: WithHealthySessionLock) => Promise<T>, gateFailure?: (error: ExecutionSessionGateErrorV2) => T): Promise<T> {
    if (!this.available(signal)) return failure;
    return this.engine.gate.withHealthySessionLock(asSessionId(this.enrollment.execution), lock =>
      ResultAsync.fromPromise((async () => {
        const loaded = await readHostState(this.engine,this.enrollment);
        if (loaded.kind !== 'loaded' || !this.available(signal)) return failure;
  ...
  async fault(boundary: JournalFaultBoundary, signal: AbortSignal): Promise<boolean> {
    if (!this.available(signal)) return false;
  ...
  async appendDelivery(reply: ReplyRef, owner: OwnerFence, signal: AbortSignal): Promise<AppendDeliveryResult> {
    if (!await this.fault('before_delivery_append',signal)) return {kind:'refused',reason:'storage_unavailable'};
    const result = await this.locked<AppendDeliveryResult>(signal,{kind:'refused',reason:'storage_unavailable'},async (state,lock) => {
      ...
      if (state.records.some(r=>r.kind==='stopped')) return {kind:'refused',reason:'stopped'};
      const view = await workView(this.engine,state);
      if (view.kind !== 'question' || view.reply !== reply) return {kind:'refused',reason:'stale_reply'};
      const old = [...state.records].reverse().find(r=>r.kind==='delivered' && r.reply===reply);
      if (old?.kind==='delivered') return {kind:'delivered',delivery:old.delivery as DeliveryRef};
  ...
  async redeliver(oldDelivery: DeliveryRef, reply: ReplyRef, owner: OwnerFence, signal: AbortSignal): Promise<RedeliverResult> {
    return this.locked<RedeliverResult>(signal,{kind:'refused',reason:'storage_unavailable'},async(state,lock)=>{
      if (!owns(state,owner)) return {kind:'stale_owner'};
      if (state.records.some(r=>r.kind==='stopped')) return {kind:'refused',reason:'stopped'};
      if (state.records.some(r=>r.kind==='captured' && r.delivery===oldDelivery)) return {kind:'refused',reason:'already_captured'};
      const old=[...state.records].reverse().find(r=>r.kind==='delivered');
      const view=await workView(this.engine,state);
      if(old?.kind!=='delivered'||old.delivery!==oldDelivery||view.kind!=='question'||view.reply!==reply) return {kind:'refused',reason:'stale_reply'};
      const delivery=this.engine.idFactory.mintEventId() as DeliveryRef;
      return await this.append(state,lock,{kind:'delivered',delivery,node:state.node,reply,epoch:owner.epoch.toString()},signal)
        ? {kind:'delivered',delivery} : {kind:'unconfirmed',reason:'commit_uncertain'};
    });
  ...
  async captureResponse(delivery: DeliveryRef, raw: unknown, owner: OwnerFence, signal: AbortSignal): Promise<CaptureResult> {
    ...
    if(decoded.kind==='refused') return {kind:'refused',reason:decoded.reason==='invalid_payload'?'invalid_delivery':decoded.reason};
    if (!await this.fault('before_capture_append',signal)) return {kind:'refused',reason:'storage_unavailable'};
    const result=await this.locked<CaptureResult>(signal,{kind:'refused',reason:'storage_unavailable'},async(state,lock)=>{
      ...
      if(delivered?.kind!=='delivered')return {kind:'refused',reason:'invalid_delivery'};
      ...
      if(previous?.kind==='captured'){
        ...
        if(prior.kind!=='decoded')return {kind:'refused',reason:'storage_unavailable'};
      ...
      const view=await workView(this.engine,state);
      if(latest?.kind!=='delivered'||latest.delivery!==delivery||view.kind!=='question'||view.reply!==delivered.reply) return {kind:'refused',reason:'invalid_delivery'};
  ...
  async prepare(response: CapturedResponse, owner: OwnerFence, signal: AbortSignal): Promise<PrepareResult> {
    if(!await this.fault('before_prepare_commit',signal))return {kind:'unconfirmed',reason:'storage_unavailable'};
    const result=await this.locked<PrepareResult>(signal,{kind:'unconfirmed',reason:'storage_unavailable'},async(state,lock)=>{
      ...
      if(response.execution!==this.enrollment.execution||captured?.kind!=='captured')return {kind:'refused',reason:'invalid_delivery'};
      const old=state.records.find(r=>r.kind==='prepared'&&r.response===response.response);
      if(old?.kind==='prepared')return {kind:'prepared',answer:preparedAnswer(state,old)};
      ...
      const view=await workView(this.engine,state);
      if(delivered?.kind!=='delivered'||view.kind!=='question'||view.reply!==delivered.reply)return {kind:'refused',reason:'invalid_delivery'};
      const call=captured.payload.calls.find(c=>c.name==='answer_work');
      let input:unknown;
      try { input=call ? JSON.parse(call.argumentsJson) : undefined; } catch { input=undefined; }
  ...
  async commitStop(owner: OwnerFence, reason: 'cancelled'|'gate_rejected'|'timeout'|'failed', detail: string, signal: AbortSignal): Promise<CommitStopResult> {
    if(!await this.fault('before_stop_commit',signal))return {kind:'refused',reason:'storage_unavailable'};
    const result=await this.locked<CommitStopResult>(signal,{kind:'refused',reason:'storage_unavailable'},async(state,lock)=>{
      if(!owns(state,owner))return {kind:'stale_owner'};
      if(state.records.some(r=>r.kind==='stopped'))return {kind:'stopped',execution:this.enrollment.execution};
      const view=await workView(this.engine,state);if(view.kind==='finished')return {kind:'refused',reason:'already_finished'};
  ...
  async recover(enrollment: HostEnrollment, owner: OwnerFence, signal: AbortSignal): Promise<RecoveryResult> {
    if(enrollment.execution!==this.enrollment.execution||enrollment.recovery!==this.enrollment.recovery)return {kind:'refused',reason:'missing'};
    return this.locked<RecoveryResult>(signal,{kind:'refused',reason:'storage_unavailable'},async state=>{
      if(!owns(state,owner))return {kind:'refused',reason:'stale_owner'};
      if(state.records.some(r=>r.kind==='stopped'))return {kind:'refused',reason:'stopped'};
      const view=await workView(this.engine,state);
      if(view.kind==='unavailable')return {kind:'refused',reason:'storage_unavailable'};
      const lastCommit=[...state.records].reverse().find(r=>r.kind==='committed');
      if(view.kind==='finished'&&lastCommit?.kind==='committed') return {kind:'settled',result:{kind:'replay',receipt:lastCommit.receipt as ReceiptRef,original:inspection(view)},view};
      if(view.kind!=='question')return {kind:'refused',reason:'corrupt'};
      const delivery=[...state.records].reverse().find(r=>r.kind==='delivered'&&r.reply===view.reply);
      if(delivery?.kind!=='delivered')return {kind:'deliver',view};
      const captured=state.records.find(r=>r.kind==='captured'&&r.delivery===delivery.delivery);
      if(captured?.kind!=='captured')return {kind:'redeliver',oldDelivery:delivery.delivery as DeliveryRef,view};
      const prepared=state.records.find(r=>r.kind==='prepared'&&r.response===captured.response);
      if(prepared?.kind==='prepared')return {kind:'replay',answer:preparedAnswer(state,prepared)};
