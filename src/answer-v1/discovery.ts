...
export async function createHostDiscovery(config: HostDiscoveryConfig, signal: AbortSignal): Promise<CreateHostDiscoveryResult> {
  ...
  try {
    ...
    if(!Keyring.safeParse(raw).success)return {kind:'refused',reason:'missing_authority',detail:'Invalid configured authority'};
  } catch(error) {
  ...
  async function entry(id:string):Promise<DiscoveredSessionEntry> {
    ...
    if(result.isErr())return {kind:'unavailable',sessionId,reason:result.error.code==='SESSION_STORE_CORRUPTION_DETECTED'
    ...
    if(enrolled.length>1)return {kind:'unavailable',sessionId,reason:'corrupt',detail:'Multiple host enrollments'};
    if(enrollment?.kind==='answer_host_recorded'&&enrollment.data.kind==='enrolled')return {kind:'host',sessionId,pointer:{formatVersion:1,executionId:id,recoveryLocator:enrollment.data.recovery}};
    return {kind:'legacy',sessionId};
  }
  return {kind:'created',scanner:{
    async scan(cursor,signal):Promise<HostScanResult>{
      if(signal.aborted)return {kind:'cancelled'};
      if(closed)return {kind:'refused',reason:'scanner_closed',detail:'Scanner is closed'};
      const offset=cursor===undefined?0:positions.get(cursor);
      if(offset===undefined)return {kind:'refused',reason:'invalid_cursor',detail:'Cursor belongs to another scanner'};
      if(!ids){
        try {ids=(await readdir(config.storage.journalRootDir,{withFileTypes:true})).filter(e=>e.isDirectory()&&/^sess_[a-z0-9]+$/.test(e.name)).map(e=>e.name).sort();}
        catch(error){return {kind:'unavailable',reason:(error as NodeJS.ErrnoException).code==='ENOENT'?'missing':'storage_unavailable',detail:'Cannot enumerate canonical sessions'};}
      }
      ...
      if(closed)return {kind:'refused',reason:'scanner_closed',detail:'Scanner closed during scan'};
      if(offset+entries.length>=ids.length)return {kind:'page',page:{kind:'end',entries}};
      const nextCursor=Object.freeze({}) as HostDiscoveryCursor;
      positions.set(nextCursor,offset+entries.length);
      return {kind:'page',page:{kind:'more',entries:entries as [DiscoveredSessionEntry,...DiscoveredSessionEntry[]],nextCursor}};
    },
    async close(signal){if(signal.aborted)return {kind:'incomplete',reason:'cancelled',detail:'Close cancelled'};closed=true;return {kind:'closed'};},
  }};
}
