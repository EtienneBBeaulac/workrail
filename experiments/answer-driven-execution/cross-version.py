...
parser.add_argument('--reader-source', choices=['archive', 'checkout'], default='archive', help='Explicitly test tracked checkout source for mutation proofs; default tests committed archives')
...
if args.reader_source == 'archive' and git('status', '--porcelain', '--', 'src', 'workflows', 'package.json', 'package-lock.json'):
...
report = {'writer': writerRevision, 'reader': readerRevision, 'checkoutHead': head, 'direction': args.direction, 'readerUsesWorkingTree': args.reader_source == 'checkout',
...
if args.reader_source == 'checkout':
    if args.direction != 'upgrade':
        raise SystemExit('Checkout reader mode requires upgrade direction')
    tracked = git('ls-files', '-z').split('\0')
    tracked = [name for name in tracked if name]
    report['readerSourceSha256'] = hashlib.sha256(b''.join(
        name.encode() + b'\0' + (ROOT / name).read_bytes() + b'\0'
        for name in sorted(tracked) if (ROOT / name).is_file())).hexdigest()
with tempfile.TemporaryDirectory(prefix='workrail-cross-version-') as temp:
    ...
    for phase, revision in [('write', writerRevision), ('read', readerRevision)]:
        ...
        tree.mkdir()
        if phase == 'read' and args.reader_source == 'checkout':
            for name in tracked:
                source = ROOT / name
                if source.is_file():
                    target_file = tree / name
                    target_file.parent.mkdir(parents=True, exist_ok=True)
                    shutil.copyfile(source, target_file)
        archive = root / (phase + '.tar')
        if not (phase == 'read' and args.reader_source == 'checkout'):
